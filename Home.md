---
type: home
---

> A small place to begin. Capture first; organize only when it helps.

## Today

- 📆 [Open today's note](obsidian://daily)
- 📩 [[90 Dashboards/Active Tasks|Active Tasks]] 📋
- 🏖️ [[90 Dashboards/Habit Tracker|Habit Tracker]]

📝✏️📔📓📗📚📑📃📄📋📆📅

📩 hg

🏖️🏝️🌈🔏💭🗯️
## Explore

- [[90 Dashboards/Media Library|Media Library]]
- [[03 Hubs/Hubs|Hubs — research, skills, projects]]

## Weekly reset

1. Clear or schedule active tasks.
2. Look at habit history without judging it.
3. Pick one item from the media library or one active hub.
